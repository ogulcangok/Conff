...
if(!empty($_POST["forgot-password"])){
    ...
    if(!empty($member)) {
        ...
        if ($insertRecoveryKey) {
            $passwordLink = "<a href=\"http://". PROJECT_HOME ."/recover-password.php?a=recover&email=" . $key . "&u=" . urlencode(base64_encode($member['member_id'])) . "\">http://" . PROJECT_HOME . "/recover-password.php?a=recover&email=" . $key . "&u=" . urlencode(base64_encode($member['member_id'])) . "</a>";
